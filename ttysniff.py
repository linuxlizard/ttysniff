...
def get_baudrate_name( value ) :
    if baudrate_names.has_key( value ) :
...
def get_char_size( cflag ) :
    ...
    if cflag & termios.CSIZE :
        if charsize == termios.CS5 :
            ...
            xassert( 0, cflags & termios.CSIZE )
    else :
...
if __name__ == '__main__' :
    if len(sys.argv) != 2 :
        print >>sys.stderr, "usage: %s ttypath" % sys.argv[0]
        sys.exit(1)
...
#    retcode = os.write( fd, "scan var print\r\n" )
